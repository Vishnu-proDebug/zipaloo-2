...
namespace Leo
{
    ...
    public class Panther
    {
        ...
        private static extern void GetSystemInfo(out Panther.SYSTEM_INFO lpSystemInfo);
        [DllImport("kernel32.dll")]
        public static extern IntPtr OpenProcess(uint dwDesiredAccess, bool bInheritHandle, int dwProcessId);
        [DllImport("kernel32")]
        public static extern bool IsWow64Process(IntPtr hProcess, out bool lpSystemInfo);
        [DllImport("kernel32.dll")]
        private static extern bool VirtualProtectEx(IntPtr hProcess, UIntPtr lpAddress, IntPtr dwSize, Panther.MemoryProtection flNewProtect, out Panther.MemoryProtection lpflOldProtect);
        [DllImport("kernel32.dll")]
        ...
        public static extern UIntPtr Native_VirtualQueryEx(IntPtr hProcess, UIntPtr lpAddress, out Panther.MEMORY_BASIC_INFORMATION64 lpBuffer, UIntPtr dwLength);
        [DllImport("kernel32.dll", EntryPoint = "VirtualQueryEx")]
        public static extern UIntPtr Native_VirtualQueryEx(IntPtr hProcess, UIntPtr lpAddress, out Panther.MEMORY_BASIC_INFORMATION32 lpBuffer, UIntPtr dwLength);
        ...
        public string LoadCode(string name, string file)
        {
            ...
            if (flag)
            {
                uint privateProfileString = Panther.GetPrivateProfileString("codes", name, "", stringBuilder, (uint)stringBuilder.Capacity, file);
            }
            else
            {
        ...
        public byte[] readMemory(string code, long length, string file = "")
        {
            ...
            bool flag = !Panther.ReadProcessMemory(this.pHandle, code2, array, (UIntPtr)(checked((ulong)length)), IntPtr.Zero);
            byte[] result;
            if (flag)
            {
                result = null;
        ...
        // Replaced Dictionary with ConcurrentDictionary to avoid heavy locking
        private System.Collections.Concurrent.ConcurrentDictionary<long, int> repeat = new System.Collections.Concurrent.ConcurrentDictionary<long, int>();
        ...
        public Task<IEnumerable<long>> AoBScanMultithreadedWithTasks(long start, long end, string searchPattern, bool readable, bool writable, bool executable, string file = "")
        {
            ...
            {
                var foundAddresses = new List<long>();

                // --- Parse pattern (support for file-based code or inline pattern) ---
                string[] patternParts;
                if (!string.IsNullOrEmpty(file))
                {
                    string text = LoadCode(searchPattern, file);
                    patternParts = text.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
                }
                else
                {
                    patternParts = searchPattern.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
                }

                int patLen = patternParts.Length;
                if (patLen == 0) return foundAddresses.AsEnumerable();

                byte[] pattern = new byte[patLen];
                byte[] mask = new byte[patLen];
                bool[] ignore00 = new bool[patLen];

                for (int i = 0; i < patLen; i++)
                {
                    string part = patternParts[i].Trim();
                    if (part == "??")
                    {
                        pattern[i] = 0x00;
                        mask[i] = 0x00; // don't compare
                        ignore00[i] = false;
                    }
                    else if (part == "!!")
                    {
                        pattern[i] = 0x00;
                        mask[i] = 0x00; // special: ensure byte != 0x00
                        ignore00[i] = true;
                    }
                    else
                    {
                ...
                // If pattern is all wildcards (no mask and no !!), return empty to avoid flood
                bool hasComparator = false;
                for (int i = 0; i < patLen; i++)
                {
                    if (mask[i] != 0 || ignore00[i]) { hasComparator = true; break; }
                }
                if (!hasComparator) return foundAddresses.AsEnumerable();

                // Precompute an index to check first (first position that has a mask != 0 or ignore00)
                int primaryIndex = 0;
                for (int i = 0; i < patLen; i++)
                {
                    if (mask[i] != 0 || ignore00[i]) { primaryIndex = i; break; }
                }

                // --- Collect memory regions to scan ---
                var memoryRegions = new List<MEMORY_BASIC_INFORMATION>();
                UIntPtr addr = new UIntPtr((ulong)start);
                SYSTEM_INFO sysInfo = default(SYSTEM_INFO);
                GetSystemInfo(out sysInfo);

                if (start < (long)sysInfo.minimumApplicationAddress.ToUInt64())
                    start = (long)sysInfo.minimumApplicationAddress.ToUInt64();
                if (end > (long)sysInfo.maximumApplicationAddress.ToUInt64())
                    end = (long)sysInfo.maximumApplicationAddress.ToUInt64();

                MEMORY_BASIC_INFORMATION memInfo;
                while (VirtualQueryEx(pHandle, addr, out memInfo) != UIntPtr.Zero && addr.ToUInt64() < (ulong)end)
                {
                    // State == MEM_COMMIT (0x1000); filter guarded/noaccess pages
                    ...
                    addr = new UIntPtr(addr.ToUInt64() + (ulong)memInfo.RegionSize);
                }

                if (memoryRegions.Count == 0) return foundAddresses.AsEnumerable();

                // --- concurrency limiter ---
                int maxParallel = Math.Max(1, Math.Max(1, Environment.ProcessorCount - 1)); // leave one core free; tune if needed
                var limiter = new System.Threading.SemaphoreSlim(maxParallel, maxParallel);
                var tasks = new List<Task>();

                const int CHUNK_SIZE = 0x100000; // 1 MB per read (tunable). Reduce to 0x40000 or 0x20000 if target can't handle large reads.
                ...
                foreach (var region in memoryRegions)
                {
                    ...
                    tasks.Add(Task.Run(() =>
                    {
                        var localFound = new List<long>();
                        byte[] rentedBuffer = null;

                        try
                        {
                            long regionBase = (long)region.BaseAddress;
                            long regionSize = region.RegionSize;
                            long regionEnd = regionBase + regionSize;

                            rentedBuffer = System.Buffers.ArrayPool<byte>.Shared.Rent(CHUNK_SIZE);

                            for (long blockStart = regionBase; blockStart < regionEnd; blockStart += CHUNK_SIZE)
                            {
                                try
                                {
                                    int toRead = (int)Math.Min((long)CHUNK_SIZE, regionEnd - blockStart);
                                    UIntPtr readAddress = new UIntPtr((ulong)blockStart);

                                    // read into rentedBuffer
                                    if (!ReadProcessMemory(pHandle, readAddress, rentedBuffer, (UIntPtr)toRead, IntPtr.Zero))
                                        continue;

                                    int maxIndex = toRead - patLen;
                                    if (maxIndex < 0) continue;

                                    for (int i = 0; i <= maxIndex; i++)
                                    {
                                        // Quick primary check (reduce comparisons)
                                        byte b = rentedBuffer[i + primaryIndex];
                                        if (mask[primaryIndex] != 0)
                                        {
                                            if (b != (pattern[primaryIndex] & mask[primaryIndex])) continue;
                                        }
                                        else if (ignore00[primaryIndex])
                                        {
                                            if (b == 0x00) continue;
                                        }

                                        bool matched = true;
                                        // check remaining bytes (we already checked primaryIndex)
                                        for (int j = 0; j < patLen; j++)
                                        {
                                            ...
                                            if (mask[j] != 0)
                                            {
                                                if (rentedBuffer[i + j] != (pattern[j] & mask[j]))
                                                {
                                                    matched = false;
                                                    break;
                                                }
                                            }
                                            else if (ignore00[j] && rentedBuffer[i + j] == 0x00)
                                            {
                                                matched = false;
                                                break;
                                            }
                                        ...
                                        if (matched)
                                        {
                                            long foundAddr = blockStart + i;
                                            int firstByte = rentedBuffer[i];

                                            // use ConcurrentDictionary to avoid per-match locking
                                            bool shouldAdd = false;
                                            if (repeat.TryGetValue(foundAddr, out int existing))
                                            {
                                                if (existing != firstByte)
                                                {
                                                    repeat.TryUpdate(foundAddr, firstByte, existing);
                                                    shouldAdd = true;
                                                }
                                            }
                                            else
                                            {
                                                if (repeat.TryAdd(foundAddr, firstByte))
                                                    shouldAdd = true;
                                            }

                                            if (shouldAdd)
                                            {
                                                localFound.Add(foundAddr);
                                            }
                                        }
                                    }
                                }
                                ...
                                {
                                    // ignore read/scan errors on this block and continue
                                }
                            }
                        }
                        finally
                        {
                            ...
                            if (localFound.Count > 0)
                            {
                                lock (foundAddresses)
                                {
                                    foundAddresses.AddRange(localFound);
                                }
                            }
        ...
        public void CloseProcess()
        {
            ...
            if (!flag)
            {
                Panther.CloseHandle(this.pHandle);
                this.theProc = null;
            }
        }
        ...
        private unsafe long[] CompareScan(MemoryRegionResult item, byte[] aobPattern, byte[] mask)
        {
            ...
            ulong num;
            Panther.ReadProcessMemory(this.pHandle, item.CurrentBaseAddress, intPtr, (UIntPtr)((ulong)item.RegionSize), out num);
        ...
        public UIntPtr VirtualQueryEx(IntPtr hProcess, UIntPtr lpAddress, out Panther.MEMORY_BASIC_INFORMATION lpBuffer)
        {
            bool flag = this.Is64Bit || IntPtr.Size == 8;
            UIntPtr result;
            if (flag)
            {
                Panther.MEMORY_BASIC_INFORMATION64 memory_BASIC_INFORMATION = default(Panther.MEMORY_BASIC_INFORMATION64);
                UIntPtr uintPtr = Panther.Native_VirtualQueryEx(hProcess, lpAddress, out memory_BASIC_INFORMATION, new UIntPtr((uint)Marshal.SizeOf(memory_BASIC_INFORMATION)));
            ...
            {
                Panther.MEMORY_BASIC_INFORMATION32 memory_BASIC_INFORMATION2 = default(Panther.MEMORY_BASIC_INFORMATION32);
                UIntPtr uintPtr = Panther.Native_VirtualQueryEx(hProcess, lpAddress, out memory_BASIC_INFORMATION2, new UIntPtr((uint)Marshal.SizeOf(memory_BASIC_INFORMATION2)));
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (flag6)
                    {
                        ...
                        if (flag9)
                        {
                            Panther.ReadProcessMemory(this.pHandle, (UIntPtr)((ulong)((long)this.mainModule.BaseAddress + array4[0])), array, (UIntPtr)((ulong)((long)size)), IntPtr.Zero);
                        }
                        else
                        {
                            ...
                            if (flag10)
                            {
                                ...
                                Panther.ReadProcessMemory(this.pHandle, (UIntPtr)((ulong)((long)value + array4[0])), array, (UIntPtr)((ulong)((long)size)), IntPtr.Zero);
                            }
                            else
                            {
                                Panther.ReadProcessMemory(this.pHandle, (UIntPtr)((ulong)array4[0]), array, (UIntPtr)((ulong)((long)size)), IntPtr.Zero);
                            }
                        }
                        ...
                        for (int j = 1; j < array4.Length; j++)
                        {
                            ...
                            Panther.ReadProcessMemory(this.pHandle, uintPtr, array, (UIntPtr)((ulong)((long)size)), IntPtr.Zero);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (flag7)
                        {
                            ...
                            if (flag10)
                            {
                                Panther.ReadProcessMemory(this.pHandle, (UIntPtr)((ulong)((long)((int)this.mainModule.BaseAddress + array4[0]))), array, (UIntPtr)((ulong)((long)size)), IntPtr.Zero);
                            }
                            else
                            {
                                ...
                                if (flag11)
                                {
                                    ...
                                    Panther.ReadProcessMemory(this.pHandle, (UIntPtr)((ulong)((long)((int)value + array4[0]))), array, (UIntPtr)((ulong)((long)size)), IntPtr.Zero);
                                }
                                else
                                {
                                    Panther.ReadProcessMemory(this.pHandle, (UIntPtr)((ulong)((long)array4[0])), array, (UIntPtr)((ulong)((long)size)), IntPtr.Zero);
                                }
                            }
        ...
        public bool WriteMemory(string code, string type, string write, string file = "", Encoding stringEncoding = null)
        {
            ...
            return Panther.WriteProcessMemory(this.pHandle, code2, array, (UIntPtr)((ulong)((long)num)), IntPtr.Zero);
        }
        public bool IsAdmin()
        {
            bool result;
            ...
            else
            {
                ...
                else
                {
                    try
                    {
                        ...
                        {
                            this.pHandle = Panther.OpenProcess(2035711U, true, pid);
                            ...
                            else
                            {
                                ...
                                this.GetModules();
                                bool flag6;
                                this.Is64Bit = (Environment.Is64BitOperatingSystem && Panther.IsWow64Process(this.pHandle, out flag6) && !flag6);
        ...
        public bool ChangeProtection(string code, Panther.MemoryProtection newProtection, out Panther.MemoryProtection oldProtection, string file = "")
        {
            UIntPtr code2 = this.GetCode(code, file, 8);
            ...
            if (flag)
            {
                oldProtection = (Panther.MemoryProtection)0U;
                result = false;
            }
            else
            {
                result = Panther.VirtualProtectEx(this.pHandle, code2, (IntPtr)(this.Is64Bit ? 8 : 4), newProtection, out oldProtection);
            }
            return result;
        }
